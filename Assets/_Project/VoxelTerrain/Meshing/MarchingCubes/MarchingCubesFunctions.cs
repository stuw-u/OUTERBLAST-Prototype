...
namespace Eldemarkki.VoxelTerrain.Meshing.MarchingCubes
{
    ...
    public static class MarchingCubesFunctions
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float ValuetInterpolate (float p1, float p2, float v1, float v2, float isolevel) {
            return p1 + (isolevel - v1) * (p2 - p1) / (v2 - v1);
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void GenerateVertexList(VoxelCorners<float> voxelDensities, VoxelCorners<float> burntVoxelDensities,
            VoxelCorners<float> brokenVoxelDensities, VoxelCorners<int3> voxelCorners,
            int edgeIndex, float isolevel, out VertexList<float3> vertexList, out VertexList<float> burntList, out VertexList<float> brokenList)
        {
            vertexList = new VertexList<float3>();
            burntList = new VertexList<float>();
            brokenList = new VertexList<float>();
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                burntList[i] = ValuetInterpolate(burntVoxelDensities[edgeStartIndex], burntVoxelDensities[edgeEndIndex], density1, density2, isolevel);
                brokenList[i] = ValuetInterpolate(brokenVoxelDensities[edgeStartIndex], brokenVoxelDensities[edgeEndIndex], density1, density2, isolevel);
            }
        }
    }
}