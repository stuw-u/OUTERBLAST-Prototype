...
#pragma warning restore 0649
        ...
        public const float CollisionOffset = 0.01f; // 0.01f
...
#pragma warning restore 0414 
        ...
        /// </summary>
        public void UpdatePhase1 (float deltaTime, float bounciness) {
            ...
            if(_movePositionDirty) {
                if(_solveMovementCollisions) {
                    ...
                    if(InternalCharacterMove(ref tmpVelocity, deltaTime, bounciness)) {
            ...
            if(_solveGrounding) {
                if(MustUnground()) {
                    ...
                    if(!GroundingStatus.IsStableOnGround && GroundingStatus.IsStableOnGround) {
            ...
            if(InteractiveRigidbodyHandling) {
                ...
                if(_attachedRigidbodyVelocity.sqrMagnitude > 0f) {
                    ...
                    if(_solveMovementCollisions) {
                        ...
                        InternalCharacterMove(ref _attachedRigidbodyVelocity, deltaTime, bounciness);
                    } else {
        ...
        /// </summary>
        public void UpdatePhase2 (float deltaTime, float bounciness) {
            // Handle rotation
            ...
            if(BaseVelocity.sqrMagnitude > 0f) {
                if(_solveMovementCollisions) {
                    InternalCharacterMove(ref BaseVelocity, deltaTime, bounciness);
                } else {
        ...
        private bool InternalCharacterMove (ref Vector3 transientVelocity, float deltaTime, float bounciness) {
            if(deltaTime <= 0f)
                return false;
        ...
        private void InternalHandleVelocityProjection (bool stableOnHit, Vector3 hitNormal, Vector3 obstructionNormal, Vector3 originalDirection,
            ...
            ref Vector3 transientVelocity, ref float remainingMovementMagnitude, ref Vector3 remainingMovementDirection, float bounciness) {
            ...
            if(stableOnHit) {
                ...
                HandleVelocityProjection(ref transientVelocity, obstructionNormal, stableOnHit, bounciness);
            } else {
                // Handle projection
        ...
        /// </summary>
        public virtual void HandleVelocityProjection (ref Vector3 velocity, Vector3 obstructionNormal, bool stableOnHit, float bounciness) {
            Vector3 reflectedVector = Vector3.Reflect(velocity, obstructionNormal);
            Vector3 projectionVector = velocity;

            if(GroundingStatus.IsStableOnGround && !MustUnground()) {
                // On stable slopes, simply reorient the movement without any loss
                if(stableOnHit) {
                    projectionVector = GetDirectionTangentToSurface(projectionVector, obstructionNormal) * projectionVector.magnitude;
                }
                // On blocking hits, project the movement on the obstruction while following the grounding plane
                    ...
                    projectionVector = GetDirectionTangentToSurface(projectionVector, obstructionUpAlongGround) * projectionVector.magnitude;
                    projectionVector = Vector3.ProjectOnPlane(projectionVector, obstructionNormal);
                }
            } else {
                if(stableOnHit) {
                    // Handle stable landing
                    projectionVector = Vector3.ProjectOnPlane(projectionVector, CharacterUp);
                    //projectionVector = GetDirectionTangentToSurface(projectionVector, obstructionNormal) * projectionVector.magnitude;
                }
            ...
            velocity = Vector3.Lerp(projectionVector, reflectedVector, bounciness);
        }
